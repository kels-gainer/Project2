...
var BootstrapModaliOS = (function() {
  var disableBodyScroll = (function() {
    // 🔗 Thijs Huijssoon https://gist.github.com/thuijssoon
  ...
  function adjustModal(e) {
    ...
    var actualViewport = window.innerHeight;
    var offsetY = modal.getBoundingClientRect().y;

    modal.style.top = 0;
    modal.style.bottom = 0;
    var screenHeight = modal.scrollHeight;

    if (
      !navigator.userAgent.match(/(iPod|iPhone|iPad)/i) ||
      Math.abs(window.orientation) !== 90 ||
      actualViewport === screenHeight
    ) {
    ...
    if (typeof e !== "undefined") {
      ...
      modal.style.top = 0 - offsetY + "px";
      modal.style.bottom = screenHeight - actualViewport + offsetY + "px";
    } else {
      if (actualViewport <= screenHeight) {
        ...
        if (
          ...
          modal.style.bottom = 0;
          modal.style.top = screenHeight - actualViewport + "px";
        } else {
          modal.style.top = 0;
          modal.style.bottom = screenHeight - actualViewport + "px";
        }
      }
      ...
      if (modal.getBoundingClientRect().y !== 0) {
        ...
        modal.style.top =
          parseInt(modal.style.top) - modal.getBoundingClientRect().y + "px";
        modal.style.bottom =
          parseInt(modal.style.bottom) + modal.getBoundingClientRect().y + "px";
      }

      if (
        actualViewport +
          parseInt(modal.style.top) +
          parseInt(modal.style.bottom) >
        screenHeight
      ) {
        // Extra bug when scrolled near the bottom

        modal.style.bottom =
          screenHeight - actualViewport - parseInt(modal.style.top) + "px";
      }
    }
  }
  ...
  $(".modal").each(function() {
    ...
    $(this).on("shown.bs.modal", function(e) {
      var activeModal = ".modal.show .modal-dialog";
      $(activeModal)[0].scrollTop = 0;
      document.documentElement.classList.add("no-scroll");
      disableBodyScroll(true, activeModal);
